...
intentions = {"Pécresse": trace["intentions"][:,0,8], "Le Pen": trace["intentions"][:,0,11]}
diff = intentions[candidates[1]]-intentions[candidates[0]]
num_wins = int(100 * np.sum(diff>0) / len(p))

x = 100 * srng.choice(diff, 100)
y = srng.normal(0,.01,len(x))
...
fig, ax  = plt.subplots()
ax.scatter(x, y, c=colors_points, s=200, edgecolor="white", zorder=2)
...
ax1 = fig.add_axes([-0.3, 0.9, 0.2, 0.2])
im = imageio.imread("~/org/roam/images/logo.png")
ax1.imshow(im)
ax1.axis("off")

fig.text(
    -0.1, 1., f"Différentiel {candidates[1]}-{candidates[0]} au premier tour", fontsize=25, fontweight="bold", fontname="Futura PT",  ha='left'
)
...
#+attr_org: :width 800
#+RESULTS:
[[file:/tmp/babel-o4KxvM/figurerQkzwf.png]]
