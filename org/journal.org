...
#+RESULTS:
[[file:/tmp/babel-OuSpkx/figurebnO2HM.png]]
...
#+attr_org: :width 600
#+RESULTS:
[[file:/tmp/babel-OuSpkx/figureEn6ZK4.png]]
...
def scatter_random(x):
    ...
    for _ in range(10_000):
        ...
        new_y = srng.normal(0,.01)
        if abs(new_y) < 0.03:
            y[idx] = new_y
    return y

candidates = ("Pécresse", "Le Pen")
intentions_r = {"Pécresse": trace["intentions"][:,8], "Le Pen": trace["intentions"][:,11]}
intentions_r = {"Pécresse": resultats_certains["Pécresse"], "Le Pen": resultats_certains["Le Pen"]}
...
fig, ax  = plt.subplots()
ax.scatter(x, y, c=colors_points, s=150, edgecolor="white", zorder=2)
ax.axvline(0, color="black", lw=2)
ax.axvline(5, ymin=0.1, color=colors[candidates[1]], lw=0.5)
ax.axvline(-5, ymin=0.1, color=colors[candidates[0]], lw=0.5)
...
ax1 = fig.add_axes([-0.3, 0.95, 0.15, 0.15])
im = imageio.imread("~/org/roam/images/logo.png")
ax1.imshow(im)
ax1.axis("off")

fig.text(
    -0.15, 1., f"Différentiel {candidates[1]}-{candidates[0]} au premier tour", fontsize=25, fontweight="bold", fontname="Futura PT",  ha='left'
)
fig.text(
    -0.15,
    0.95,
    f"08/12/2021 | BVA pour Orange et RTL | N=894 | Certains d'aller voter et sûrs de leur choix",
    fontsize=13,
    fontweight="normal",
    fontname="Futura PT",
    color="darkgray",
...
#+attr_org: :width 800
#+RESULTS:
[[file:/tmp/babel-OuSpkx/figureR0HoYQ.png]]
